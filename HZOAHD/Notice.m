       ...
       // employeeId = [NSUtil addLT:employeeId];
        ...
        if (sqlite3_prepare_v2(hzoaDB, query_stmt, -1, &statement, NULL) == SQLITE_OK) 
        {
            while (sqlite3_step(statement) == SQLITE_ROW) 
            {
                ...
                // fileId
                NSString *fileIdField = [[NSString alloc] initWithUTF8String:(const char *)sqlite3_column_text(statement, 11)];
                notice.fileId = fileIdField;
                // fileName
                NSString *fileNameField = [[NSString alloc] initWithUTF8String:(const char *)sqlite3_column_text(statement, 12)];
                notice.fileName = fileNameField;
