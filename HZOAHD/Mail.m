...
//添加到草稿箱
+ (NSString *)serviceSaveEmail:(Mail *) mail {
...
+ (NSMutableArray *) getLocalSenderEmail:(id) employeeId {
    ...
    {
        NSString *querySQL = [NSString stringWithFormat:@"SELECT * FROM EMAIL WHERE SENDER = \"%@\" ORDER BY DATE DESC;", employeeId];
...
+ (NSMutableArray *) getLocalReciveEmail:(id) employeeId {
    ...
    if (sqlite3_open(dbpath, &hzoaDB) == SQLITE_OK) 
    {
        ...
        NSString *querySQL = [NSString stringWithFormat:@"SELECT * FROM EMAIL WHERE RECIVER LIKE '%%%%%@%%' OR COPYID LIKE '%%%%%@%%';", employeeId, employeeId];
        const char *query_stmt = [querySQL UTF8String];
        if (sqlite3_prepare_v2(hzoaDB, query_stmt, -1, &statement, NULL) == SQLITE_OK) 
        {
            while (sqlite3_step(statement) == SQLITE_ROW) 
